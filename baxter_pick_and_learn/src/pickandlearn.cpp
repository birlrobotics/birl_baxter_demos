...
int main(int argc, char **argv)
{
    ...
    float position[3], obj_position[3], orientation[4], obj_orientation[4], tmp_position[3], tmp_orientation[4];//New Created FLAG
    spinner.start();
    // Learning
    bool learning = true;
    robot->release();
        ...
        else if(state == 3 && camera->isResultAvailable())
        {
            ...
            else
            {
                ...
                if(camera->getClosestMatchApproach(result, pieces, position[2], obj_position, obj_orientation, match))
                {
                    ...
                    //robot->getPosition(tmp_position);
                    //robot->getOrientation(tmp_orientation);
                    //tmp_position[2] += (tmp_position[2] > 0.3) ? -0.05 : 0.05;//New Created FLAG
                    //robot->moveTo(tmp_position, tmp_orientation);
                    state = 2;
                }
                else
                {
                    ...
                    {
                        robot->getPosition(tmp_position);
                        if(robot->moveTo(tmp_position, obj_orientation))
                            state = 2;
                        else
                            state = 4;// New Created FLAG
                        // Smooth approach
                        obj_position[0] -= 0.1 - 0.08*pass/MAX_PASS;
                        obj_position[2] += 0.1 - 0.08*pass/MAX_PASS;
                        if(robot->moveTo(obj_position, obj_orientation))//V2_0
                            state = 2;
                        else
                            state = 4;
                    }
